...
Расшифровать селектор `$parent[UserControl]` можно так: в родительском элементе (parent) взять текущий дочерний UserControl и связать его со свойством **SomeString**. Этот селектор является своеобразным трюком AvaloniaUI - если вместо него указать просто имя свойства SomeString, внешнее связывание (в родительской XAML разметке) работать не будет.

>Обратиться к свойству элемента-предшественника также можно используя связывание через **RelativeSource**:
>
>```csharp
...
>```
>
>Использование KeyboardButton (производный от UserControl) осуществляется так:
>
>```csharp
...
## Передать событие родительскому элементу
