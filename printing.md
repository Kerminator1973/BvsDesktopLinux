...
Следует заметить, что ориентироваться на конкретный текст в строке выводимой командой lpstat нельзя, т.к. в зависимости от локализации текст может быть разным.

Поместить файл в очередь печати принтера можно командой `lpr [имя файла]`. Отметим, что если идентификатор принтера не указывается явным образом, мы должны указать, что конкретный принтер является принтером по умолчанию в настройках операционной системы.

Кроме утилит, в дистрибутивах с поддержкой Desktop часто реализованы утилиты для доступа к очередям принтера.
...
Если мы хотим получать состояние принтера не повышая привелегий (sudo), то нам следует создать в папке /etc/udev/rules.d файл с именем «70-cashino-usb.rules» и добавить в него следующую команду:

``` cfg
SUBSYSTEM=="usb", ATTR{idVendor}=="0FE6", ATTR{idProduct}=="811E", MODE="666"
```

Эта команда указывает, что к указанному USB-устройству возможен полный доступ.

Взаимодействие с USB HID на C# подробно описано в документе по [ссылке](./usbhid.md)
