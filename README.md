...
- выбор картинки в зависимости от свойства (возможно, что проблема решаема посредством Binding Classes: https://stackoverflow.com/questions/68979876/how-to-simulate-datatrigger-with-avalonia и https://docs.avaloniaui.net/docs/data-binding/binding-classes)
- установка фонового цвета строки, в зависимости от текста ячейки DataGrid

Таким образом, практическая целесообразность использования этого механизма, пока находится под вопросом.
...
Однако, у приведённого выше примера есть один **огромный недостаток**: если свойства строки были изменены, а затем содержимое таблицы было сброшено и были загружены другие данные, то изменённое свойство сохранится. Как результат, в данном примере, в новой выборке красный фон будет установлен у случайной строки. Смотри [Issue 14](https://github.com/Kerminator1973/BvsDesktopLinux/issues/14). Попытки сортировки строк DataGrid по щелчку на колонке также приводят к ужасающим глюкам.
