...
>В 2025 году появился интересный online-IDE [Xaml.io](https://xaml.io/), который позволяет разрабатывать XAML-приложения без необходимости установки .NET Core и Visual Studio/Visual Studio Code на рабочий компьютер.
...
Возможно, что так же требуется определить и префикс "vm:" (пространство имён, в котором определён класс MainWindowViewModel):

``` csharp
xmlns:vm="using:approve.ViewModels"
```
